...
**Model Specification:**
```math
\ln(P_{ibt}) = \alpha + \tau (S_i \times \Delta \text{Policy}_{st}) + \beta_1 S_i + \beta_2 \Delta \text{Policy}_{st}
+ \gamma_1 d_{ic} + \gamma_2 (S_i \times d_{ic}) + \gamma_3 (d_{ic} \times \Delta \text{Policy}_{st}) + \gamma_4 (S_i \times d_{ic} \times \Delta \text{Policy}_{st})
...
## 6. Future Work: Heterogeneity Analysis

To address the dampening of baseline LATE significance under heavily saturated models, future work involves:
1. **Relaxing Standard Error Penalty**: Examining robust standard error formulations via single-way clustering block variants (e.g., segment-only vs two-way).
2. **Timeframe Heterogeneity**: Segregating housing transactions across specific temporal sub-samples (Pre-COVID-19 vs COVID-Era) to isolate era-specific shifts in amenity value capitalization.
3. **Model Refinements**: Incorporating structural housing controls via updated Census ACS queries, testing thresholds in tuition gap treatment variables, and splitting cross-sections by urban-rural densities.